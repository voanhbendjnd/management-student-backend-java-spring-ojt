...
public class EnrollmentService {
    ...
    public void enroll(Long courseId) {
        ...
        try {
            ...
            // concurrent update slot
            var updated = this.courseRepository.updateSlotStudents(courseId, course.getVersion());
            if (updated <= 0) {
                throw new DenyException("Course is full!");
            }
            // update student credits after successful enrollment
            student.setCountCredits((student.getCountCredits() == null ? 0 : student.getCountCredits()) +
                    (course.getCredits() == null ? 0 : course.getCredits()));
